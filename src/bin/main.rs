...
fn main() {
    ...
    let params = match ConfigElmo::new(&args) {
    ...
    if let Err(e) = elmo_train.run_training(&mut iters.next().unwrap(), &mut iters.next(), params.learning_rate, params.max_iter, model, params.token_vocab_size, &mut vars) {
        panic!("{}", e);
    };
